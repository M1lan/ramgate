...
check_tool() {
    ...
    if ! has "$t"; then CHECK_STATE=missing; return; fi
    case "$t" in
        bash)
            ((BASH_VERSINFO[0] > 5 \
              || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] > 3) \
              || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] == 3 && BASH_VERSINFO[2] >= 15))) \
                || { CHECK_STATE=outdated; CHECK_NOTE="need GNU >= 5.3.15, got $BASH_VERSION"; } ;;
    esac
}
...
project_checks() {
    ...
    if [[ -x /opt/homebrew/bin/bash ]]; then
        ...
        state=ok; note="/opt/homebrew/bin/bash ($bv) -- re-exec target"
    else
        state=missing; note='/opt/homebrew/bin/bash absent -- brew install bash'
    fi
    project_rows+=("$state|homebrew bash|$note")
    # macOS build: WARN (never error) if not the tested 26.x target.
    local osv; osv=$(fact_osver)
    if [[ $osv == 26.* ]]; then state=ok; note="macOS $osv (tested target)"
    else state=warn; note="macOS $osv -- ramgate is tuned for 26.x, YMMV"; fi
    ...
    if [[ -n "$(fact_localbin_ok)" ]]; then
        state=ok; note='~/.local/bin on PATH (installed bins resolve)'
    else
        state=warn; note='~/.local/bin not on PATH -- add it before `just install`'
    fi
    ...
    if (( bins_ok )); then state=ok; note='bin/ram-xray + bin/ram-guard present'
    else state=missing; note='a ramgate binary is missing from bin/'; fi
...
collect_missing() {
    ...
    for t in "${REQUIRED[@]}"; do
        check_tool "$t"; [[ $CHECK_STATE == ok ]] || MISSING_REQ+=("$t")
    done
    for t in "${RECOMMENDED[@]}" "${OPTIONAL[@]}"; do
        check_tool "$t"; [[ $CHECK_STATE == ok ]] || MISSING_OPT+=("$t")
    done
}
...
cmd_install() {
    ...
    (( failed )) && { printf '%s%s some installs failed -- re-run: just doctor%s\n' "$C_RED" "$I_MISS" "$C_RESET"; return 1; }
